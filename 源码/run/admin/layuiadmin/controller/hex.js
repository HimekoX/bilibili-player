layui.define(['treeSelect', 'layer', 'jquery', 'form', 'admin', 'setter', 'table', 'authtree', 'upload', 'laydate'], function (exports) {
    ...
    let hex = {
        ...
        popup(url, fields, done, values = {}, area = '660px', saveTitle = '添加', editTitle = '修改') {
            ...
            layer.open({
                ...
                shade: 0.3,
                content: d,
                title: values.hasOwnProperty('id') ? editTitle : saveTitle,
                btn: ['确认', '取消'],
                //  shadeClose: true,
                area: area,
                maxmin: true,
            ...
            fields.forEach(item => {
                ...
                switch (item.type) {
                    case "input":
                        ...
                        $(elem + ' input[name=' + item.name + ']').change(function () {

                            if (item.name == 'refresh') {
                                let s = $(this).val();
                                if (s > 0) {
                                    //开启自动刷新
                                    layer.msg("已生效，执行周期：" + s + "秒");
                                    hex.property.map['refresh'] = setInterval(() => {
                                        $(elem.replace('.query', '') + '.layui-laypage-btn').click();
                                    }, s * 1000);
                                } else {
                                    layer.msg("已关闭");
                                    clearInterval(hex.property.map['refresh']);
                                }
                    ...
                    case "select":
                        formHtml.append('<div class="layui-input-inline" ' + width + '>\n' +
                            '                    <select lay-filter="' + item.name + '" name="' + item.name + '" ' + (item.search == true ? ' lay-search=""' : '') + '><option value="">' + item.title + '</option></select>\n' +
                            '                        </div>');
                        ...
                        if (item.hasOwnProperty('dict')) {
                            this.getDict(item.dict, res => {
                                ...
                                boxesObject[item.name] = xmSelect.render({
                                    ...
                                    on: function (arr) {
                                        console.log(arr)
                                        if (arr.change.length > 0 && item.auto === true) {
                                            //自动点击查询按钮
                                            setTimeout(() => {
                                                $(elem + ' .queryBtn').click();
                                            }, 100);
        ...
        getDict(dict, done, keywords = '', cache = true) {
            // if (cache) {
            ...
            admin.req({
                ...
                method: "post",
                data: {dict: dict, keywords: keywords},
                done: res => {
                    if (keywords == '' && cache) {
        ...
        getDictSync(dict, keywords = '', cache = true) {
            // if (cache) {
            //     let data = localStorage.getItem(dict);
            //     if (data) {
        ...
        tableChange(saveUrl, id, field, value, done = null) {
            let json = {}
            json[field] = value;
            json.id = id;
            admin.req({
        ...
        renderTable(filter, url, elem, cols, done, saveUrl = null, height = null, limit = 15, limits = [15, 20, 30, 40, 50, 100, 200, 500]) {
            let tableInstance, self = this;

            //注册全局修改事件
            window.tableChange = function (saveUrl, id, field, value, done = null) {
                self.tableChange(saveUrl, id, field, value, done);
            };

            //注册字典
            for (let i = 0; i < cols[0].length; i++) {
                let field = cols[0][i].field, dictType = cols[0][i].dictType, title = cols[0][i].title,
                    dictText = cols[0][i].dictText,
                ...
                if (cols[0][i].hasOwnProperty('dict') && dictType !== undefined) {
                    ...
                    cols[0][i].templet = function (item) {
                        ...
                        switch (dictType) {
                            case "switch":
                                s = '<input type="checkbox" lay-filter="' + filter + '-checkbox" title="' + dictText + '" data-field="' + field + '" data-id="' + item.id + '" ' + (item[field] === 1 ? "checked" : "") + '>';
                                break;
                            case "image":
                                s = '<img class="' + filter + '-image" style="height: 100%;cursor: pointer;max-width: 100%;" src="' + url + item[field] + '" data-id="' + item.id + '">';
                                break;
                            case "video":

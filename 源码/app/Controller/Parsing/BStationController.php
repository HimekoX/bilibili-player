...
use App\Controller\ParsingBaseController;
use App\Utils\RequestUtil;

/**
 * B站弹幕解析库
 * Class BStationController
 * @package App\Controller\Parsing
 * @Controller("/parsing/bstation")
 */
class BStationController extends ParsingBaseController
{
    /**
     * 解析接口
     * @RequestMapping("/")
    ...
    public function parsing()
    {
        ...
        if ($cid > 0 or ($cid > 0 and $av > 0)) {
            $xml = $this->curl_get('https://api.bilibili.com/x/v1/dm/list.so?oid=' . $cid);

            return $this->xml_json($xml);
        } elseif ($av > 0 and $cid <= 0) {
            $cid = $this->get_cid($av, $p);
            $xml = $this->curl_get('https://api.bilibili.com/x/v1/dm/list.so?oid=' . $cid);
            return $this->xml_json($xml);
        } else {
    ...
    private function curl_get($url, $gzip = 1)
    {
        $curl = curl_init($url);
    ...
    private function xml_json($xml)
    {
        ...
        foreach ($danmuku as $k => $v) {
            ...
            $json['danmuku'][$k][] = $this->hex2rgb($v[1][6]); //字体的颜色
    ...
    private function get_cid($av, $p)
    {
        $data = json_decode(RequestUtil::curl_request('https://api.bilibili.com/x/player/pagelist?aid=' . $av), true);
